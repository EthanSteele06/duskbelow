...
export function VendorScreen() {
  ...
  return (
    <div className="flex min-h-full flex-col p-3 gap-3">
      ...
      <p className="font-body text-sm text-muted-foreground -mt-2">A grizzled smith eyes your purse.</p>
          ...
          return (
            <div key={it.id} className="border-2 border-black bg-card p-2">
              ...
              <div className="mt-2 flex gap-2">
                <button onClick={() => buy(it.id)} disabled={!canBuy} className="pixel-btn pixel-btn-gold !text-[8px] disabled:opacity-40">Buy</button>
                {it.kind === "potion" && owned > 0 && (
                  <button onClick={() => use(it.id)} className="pixel-btn !text-[8px]">Use ({owned})</button>
                )}
                {owned > 0 && it.kind !== "potion" && (
