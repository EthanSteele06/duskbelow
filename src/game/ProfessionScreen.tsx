...
import { PROFESSIONS, RECIPES, MATERIALS, profXpForLevel, VENDOR_ITEMS } from "@/game/data";
...
export function ProfessionScreen() {
  ...
  return (
    <div className="flex min-h-full flex-col p-3 gap-3">
      ...
      <p className="font-body text-sm text-muted-foreground -mt-2">Work between dungeon runs. Sell what you don't need.</p>

      {!p.profession ? (
        <>
          <p className="pixel text-[9px] text-foreground mt-1">Choose a profession (permanent for this run):</p>
  ...
  function Materials({ onSell }: { onSell: (id: string) => void }) {
    ...
    return (
      <div>
        ...
        {entries.length === 0 && <p className="font-body text-sm text-muted-foreground">No materials yet. Slay things below.</p>}
